...
# Domain Abfrage
DOMAIN = cConfig().getSetting('plugin_' + SITE_IDENTIFIER + '.domain', 'www.kool.to') # Domain Auswahl über die xStream Einstellungen möglich
...
def showEntries(entryUrl=False, sGui=False):
    ...
    for i in aResults:
        ...
        if 'description' in i and i['description'] != '': oGuiElement.setDescription(
            i['description'])  # Suche nach Desc, wenn es nicht leer dann setze GuiElement
        # sThumbnail = i['poster']
        ...
        if 'backdrop' in i and i['backdrop'] != '': oGuiElement.setFanart(
            i['backdrop'])  # Suche nach Fanart, wenn es nicht leer dann setze GuiElement
